...
# Danh sách nguồn dữ liệu (Đã thêm link AdGuard)
URLS = [
    "https://abpvn.com/filter/abpvn-i6PlHq.txt",
...
for URL in URLS:
    ...
    try:
        ...
        print(f"Lỗi khi xử lý file từ {URL}: {e}")

# Ghi ra file blocklist.txt chuẩn AdGuard Home
try:
    with open("blocklist.txt", "w", encoding="utf-8") as f:
        f.write("! Title: Combined Clean DNS Blocklist\n")
        f.write(f"! Total domains: {len(domains)}\n\n")
        for domain in sorted(domains):
            f.write(f"||{domain}^\n")

    print(f"Thành công! Đã trích xuất tổng cộng {len(domains)} domain duy nhất.")
except Exception as e:
